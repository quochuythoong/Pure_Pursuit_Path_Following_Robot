...
cameraMatrix = np.array([[720.54487383, 0.0, 286.45899308],
                         [0.0, 720.64189824, 228.84466614],
                         [0.0, 0.0, 1.0]])
dist = np.array([[0.07157306, -0.35613325, -0.00412231, -0.00323913, 0.30004316]])
...
while True:
    ...
    # Apply undistortion
    undistorted_frame = cv2.undistort(frame, cameraMatrix, dist)
    ...
    if ids is not None:
        ...
        for i, corner in enumerate(corners):
            ...
            cv2.circle(undistorted_frame, (cx, cy), 5, (0, 255, 0), -1)  # Green circle at the center
            ...
            cv2.arrowedLine(undistorted_frame, (cx, cy), (end_x, end_y), (0, 255, 0), 2)  # Green arrow
            ...
            cv2.putText(undistorted_frame, f"ID: {ids[i][0]} X: {cx}, Y: {cy}", 
                        (cx + text_offset_x, cy - text_offset_y), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
            cv2.putText(undistorted_frame, f"Angle: {angle:.2f} deg", 
                        (cx + text_offset_x, cy + text_offset_y), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    ...
    if len(clicked_points) > 0:
        for i in range(len(clicked_points)):
            ...
            cv2.circle(undistorted_frame, clicked_points[i], 5, (0, 0, 255), -1)  # Red circle for clicked point

            # If there is more than one point, connect the points with lines
            if i > 0:
                cv2.line(undistorted_frame, clicked_points[i - 1], clicked_points[i], (255, 0, 0), 2)  # Blue line between points

        # Automatically connect the last dot to the first dot if there are 3 or more points
        if len(clicked_points) >= 3:
            cv2.line(undistorted_frame, clicked_points[-1], clicked_points[0], (255, 0, 0), 2)  # Blue line to close the shape

    # Draw buttons
    draw_buttons(undistorted_frame)

    # Display the frame with detection and vector
    cv2.imshow("2D ArUco Marker Detection and Vector", undistorted_frame)
