...
def find_closest_point(current_position, waypoints, look_ahead_distance, error_tolerance=1.0):
    ...
    for i, point in enumerate(waypoints):
        ...
        # Check if this distance is closest to the desired look-ahead distance within error tolerance
    ...
    # Clear points up to the closest index, if found
    if closest_index >= 0:
...
while True:
    ...
    if ids is not None:
        ...
        for i, corner in enumerate(corners):
            ...
            # Calculate the center of the marker (mean of the corner points) and invert Y-coordinate
            cx = int(np.mean(points[:, 0]))  # X coordinate of the center
            cy = frame_height - int(np.mean(points[:, 1]))  # Y coordinate of the center
            ...
            vec_y = points[0][1] - points[1][1]  # Y direction between top-left and top-right corners
            ...
            cv2.putText(frame, f"ID: {ids[i][0]} X: {cx}, Y: {cy}", 
                        (cx + text_offset_x, frame_height - (cy - text_offset_y)), 
    ...
    if len(clicked_points) > 0:
        for i in range(len(clicked_points)):
            ...
            cv2.circle(frame, (clicked_points[i][0], frame_height - clicked_points[i][1]), 5, (0, 0, 255), -1)  # Red circle for clicked point
            ...
            if i > 0:
                cv2.line(frame, (clicked_points[i - 1][0], frame_height - clicked_points[i - 1][1]), 
                         (clicked_points[i][0], frame_height - clicked_points[i][1]), (255, 0, 0), 2)  # Blue line between points
  ...
  # Blue line to close the shape
    ...
    step_distance = 1.0 
    if len(clicked_points) > 2 and center_coordinate != 0:
        interpolated_waypoints = interpolate_waypoints(clicked_points, step_distance)
        ...
        if closest_point is not None:
            # Call the calculate_signed_AH_and_projection function with center_coordinate, end_point_arrow, and closest_point
